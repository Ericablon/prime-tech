...
export function NewOrderPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><div><h1 className="pt-page-title">Nova Ordem de Serviço</h1><p className="pt-page-subtitle">Entrada realizada pelo atendimento. O técnico recebe a OS na fila.</p></div>
    {error && <p role="alert" className="pt-card">{error}</p>}
    <form className="pt-card grid gap-5 md:grid-cols-2" onSubmit={async (e) => {
      e.preventDefault();
      setBusy(true); setError(""); try { const order = await createOrder({ client_id: clientId, equipment_id: equipmentId, intake_type: intakeType, reported_issue: issue, priority });
      navigate(`/ordens/${order.id}`); } catch(error) { setError(error && typeof error === "object" && "message" in error ? String(error.message) : "Não foi possível abrir a OS"); } finally {setBusy(false);}
    }}>
      ...
      <div><label className="pt-label">Equipamento</label><select className="pt-input" value={equipmentId} onChange={(e) => setEquipmentId(e.target.value)} required>{equipment.filter((eq) => eq.client_id === clientId).map((eq) => <option key={eq.id} value={eq.id}>{equipmentCode(eq.technical_number)} · {eq.category} • {eq.brand} {eq.model}</option>)}</select></div>
      ...
      <div className="md:col-span-2 flex justify-end"><button className="pt-btn-primary" disabled={busy || !clientId || !equipmentId}>Gerar Ordem de Serviço</button></div>
    </form>
  </div>;
}
