...
export function StockPage(){
  ...
  const canReserve=can(user,'stock.reserve'); const canConsume=can(user,'stock.consume'); const canAdjust=can(user,'stock.adjust'); const canMove=canReserve||canConsume||canAdjust;
  ...
  async function submit(event:FormEvent){event.preventDefault();const physical=Number(form.physical.replace(',','.'));const minimum=Number(form.minimum.replace(',','.'));const cost=Number(form.cost.replace(',','.'));const sale=Number(form.sale.replace(',','.'));if(!form.sku.trim()||!form.name.trim())return setLocalError('Informe SKU e nome do item.');if(![physical,minimum,cost,sale].every(Number.isFinite)||physical<0||minimum<0||cost<0||sale<0)return setLocalError('Quantidades e valores devem ser válidos e não negativos.');setSaving(true);setLocalError('');try{await addStockItem({sku:form.sku.trim(),name:form.name.trim(),quantity:physical,reserved_quantity:0,minimum_quantity:minimum,physical,reserved:0,minimum,cost_price:cost,sale_price:sale,active:true});closeNew();}catch(cause){setLocalError(cause instanceof Error?cause.message:'Não foi possível cadastrar o item.');}finally{setSaving(false);}}
  ...
  async function submitMovement(event:FormEvent){event.preventDefault();if(!selectedItem)return;const quantity=Number(movementForm.quantity.replace(',','.'));if(!Number.isFinite(quantity)||quantity===0)return setLocalError('Informe uma quantidade válida e diferente de zero.');if(movementType!=='adjust'&&quantity<0)return setLocalError('A quantidade deve ser maior que zero.');if(movementType==='reserve'&&!canReserve)return setLocalError('Seu perfil não pode reservar estoque.');if((movementType==='consume'||movementType==='release')&&!canConsume&&!canReserve)return setLocalError('Seu perfil não pode consumir ou liberar reservas.');if(movementType==='adjust'&&!canAdjust)return setLocalError('Seu perfil não pode ajustar estoque.');if(mode!=='supabase'||!supabase)return setLocalError('Movimentações transacionais exigem Supabase.');setSaving(true);setLocalError('');const common={p_stock_item_id:selectedItem.id,p_quantity:Math.abs(quantity),p_service_order_id:movementForm.orderId||null,p_notes:movementForm.notes.trim()||null,p_idempotency_key:crypto.randomUUID()};try{if(movementType==='reserve'){const{error:e}=await supabase.rpc('reserve_stock',common);if(e)throw e;}else if(movementType==='release'){const{error:e}=await supabase.rpc('release_stock_reservation',common);if(e)throw e;}else if(movementType==='consume'){const{error:e}=await supabase.rpc('consume_stock',common);if(e)throw e;}else{const{error:e}=await supabase.rpc('adjust_stock',{p_stock_item_id:selectedItem.id,p_delta:quantity,p_notes:movementForm.notes.trim()||null,p_idempotency_key:crypto.randomUUID()});if(e)throw e;}await refresh();closeMovement();}catch(cause){setLocalError(cause instanceof Error?cause.message:'Não foi possível registrar a movimentação.');}finally{setSaving(false);}}
