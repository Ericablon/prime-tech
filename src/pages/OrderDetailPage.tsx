...
import { money, orderCode, dateTime, equipmentCode } from "../lib/formatters";
...
export function OrderDetailPage() {
  const { id } = useParams();
  const { user } = useAuth();
  const { orders, clients, equipment, updateTechnical, transitionOrder, recordContact, history, loading } = usePrimeTech();
  ...
  return <div className="space-y-6">{notice && <p role="status" className="pt-card">{notice}</p>}
    ...
    {((canTech && ["waiting_technician", "diagnosis"].includes(order.status)) || commercialQuote) ? <section className="pt-card"><h2 className="pt-section-title">Diagnóstico e orçamento técnico</h2><p className="pt-section-description">{commercialQuote ? "Revise os valores e registre o envio ao cliente após realizar o contato." : "Ao enviar, o orçamento retorna ao Comercial para apresentar ao cliente."}</p><div className="mt-5 grid gap-4 md:grid-cols-3"><div className="md:col-span-2"><label className="pt-label">Diagnóstico</label><textarea className="pt-input min-h-28" disabled={commercialQuote} value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} /></div><div><label className="pt-label">Prazo estimado (dias)</label><input className="pt-input" type="number" min="1" disabled={commercialQuote} value={days} onChange={(e) => setDays(Number(e.target.value))} /></div></div>
    ...
    <section className="pt-card"><h2 className="pt-section-title">Ações do fluxo</h2><label className="block mt-3"><span className="pt-label">Observações da ação / atendimento ao cliente</span><textarea className="pt-input" value={actionNotes} onChange={e=>setActionNotes(e.target.value)} placeholder="Descreva a aprovação, serviço executado, contato ou entrega" /></label><div className="mt-4 flex flex-wrap gap-3">
      {canApproval && order.status === "waiting_customer" && <><button className="pt-btn-primary" disabled={busy} onClick={() => run(() => transitionOrder(order.id, "approved", actionNotes || "Cliente aprovou o orçamento"), "Status atualizado")}>Cliente aprovou</button><button className="pt-btn-danger" disabled={busy} onClick={() => run(() => transitionOrder(order.id, "cancelled", actionNotes || "Cliente recusou o orçamento"), "Status atualizado")}>Cliente recusou</button></>}
      ...
      {canTech && order.status === "in_repair" && <><button className="pt-btn-secondary" disabled={busy} onClick={() => run(() => transitionOrder(order.id, "waiting_part", "Aguardando peça"), "Status atualizado")}>Aguardando peça</button><button className="pt-btn-primary" disabled={busy || actionNotes.trim().length < 3} onClick={() => run(() => transitionOrder(order.id, "ready_for_pickup", actionNotes), "Status atualizado")}>Concluir e devolver ao Comercial</button></>}
      ...
      {canDelivery && order.status === "ready_for_pickup" && <button className="pt-btn-primary" disabled={busy} onClick={() => run(() => transitionOrder(order.id, "delivered", actionNotes || "Equipamento entregue ao cliente"), "Status atualizado")}>Confirmar entrega</button>}
      ...
      {canApproval && <button disabled={busy || actionNotes.trim().length < 3} className="pt-btn-secondary" onClick={()=>run(()=>recordContact(order.id,actionNotes),"Contato registrado")}>Registrar contato com cliente</button>}
      {canApproval && ["budget_ready","waiting_customer"].includes(order.status) && <button disabled={busy || actionNotes.trim().length < 3} className="pt-btn-secondary" onClick={()=>run(()=>transitionOrder(order.id,"diagnosis",actionNotes),"Devolvido para avaliação")}>Devolver ao Técnico para revisão</button>}
    </div></section>
    <PaymentPanel orderId={order.id}/>
    <section className="pt-card space-y-3"><h2 className="pt-section-title">Histórico da OS</h2>{history.filter(h=>h.service_order_id===order.id).map(h=><div key={h.id} className="border-b border-border pb-3"><p className="text-xs text-muted">{dateTime.format(new Date(h.changed_at))}</p><p className="whitespace-pre-wrap">{h.notes}</p><span className="text-xs text-muted">{h.from_status} → {h.to_status}</span></div>)}</section>
  </div>;
}
