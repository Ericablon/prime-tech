...
export function OrderDetailPage() {
  ...
  return <div className="space-y-6">
    ...
    {canTech && ["waiting_technician", "diagnosis"].includes(order.status) ? <section className="pt-card"><h2 className="pt-section-title">Diagnóstico e orçamento técnico</h2><p className="pt-section-description">Ao enviar, a OS irá para “Aguardando cliente”.</p><div className="mt-5 grid gap-4 md:grid-cols-3"><div className="md:col-span-2"><label className="pt-label">Diagnóstico</label><textarea className="pt-input min-h-28" value={diagnosis} onChange={(e) => setDiagnosis(e.target.value)} /></div><div><label className="pt-label">Prazo estimado (dias)</label><input className="pt-input" type="number" min="1" value={days} onChange={(e) => setDays(Number(e.target.value))} /></div></div>
      <div className="mt-5 rounded-2xl border border-border p-4"><h3 className="font-semibold">Itens do orçamento</h3><div className="mt-3 grid gap-3 md:grid-cols-5"><select className="pt-input" value={newItem.kind} onChange={(e) => setNewItem((s) => ({ ...s, kind: e.target.value as ItemKind }))}><option value="service">Serviço</option><option value="part">Peça</option></select><input className="pt-input md:col-span-2" placeholder="Descrição" value={newItem.description} onChange={(e) => setNewItem((s) => ({ ...s, description: e.target.value }))} /><input className="pt-input" type="number" min="1" value={newItem.quantity} onChange={(e) => setNewItem((s) => ({ ...s, quantity: Number(e.target.value) }))} /><input className="pt-input" type="number" min="0" step="0.01" placeholder="Valor" value={newItem.unit_price} onChange={(e) => setNewItem((s) => ({ ...s, unit_price: Number(e.target.value) }))} /></div><button type="button" className="pt-btn-secondary mt-3" onClick={() => { if (!newItem.description.trim()) return; setItems((s) => [...s, newItem]); setNewItem({ kind: "service", description: "", quantity: 1, unit_price: 0, cost_price: 0 }); }}>Adicionar item</button>
      <div className="mt-4 space-y-2">{items.map((item, index) => <div key={`${item.description}-${index}`} className="flex items-center justify-between rounded-xl bg-surface-2 px-3 py-2 text-sm"><span>{item.kind === "service" ? "Serviço" : "Peça"} • {item.description} × {item.quantity}</span><div className="flex items-center gap-3"><strong>{money.format(item.quantity * item.unit_price)}</strong><button className="text-red-400" onClick={() => setItems((s) => s.filter((_, i) => i !== index))}>remover</button></div></div>)}</div><div className="mt-4 text-right font-semibold">Total: {money.format(total)}</div></div>
      <div className="mt-5 flex justify-end"><button className="pt-btn-primary" onClick={() => updateTechnical(order.id, diagnosis, days, items)}>Enviar orçamento para atendimento</button></div>
    </section> : null}
    ...
    <section className="pt-card"><h2 className="pt-section-title">Ações do fluxo</h2><div className="mt-4 flex flex-wrap gap-3">
      {canApproval && order.status === "waiting_customer" && <><button className="pt-btn-primary" onClick={() => transitionOrder(order.id, "approved", "Cliente aprovou o orçamento")}>Cliente aprovou</button><button className="pt-btn-danger" onClick={() => transitionOrder(order.id, "cancelled", "Cliente recusou o orçamento")}>Cliente recusou</button></>}
      {canTech && order.status === "approved" && <button className="pt-btn-primary" onClick={() => transitionOrder(order.id, "in_repair", "Manutenção iniciada")}>Iniciar manutenção</button>}
      {canTech && order.status === "in_repair" && <><button className="pt-btn-secondary" onClick={() => transitionOrder(order.id, "waiting_part", "Aguardando peça")}>Aguardando peça</button><button className="pt-btn-primary" onClick={() => transitionOrder(order.id, "ready_for_pickup", "Manutenção concluída")}>Concluir manutenção</button></>}
      {canTech && order.status === "waiting_part" && <button className="pt-btn-primary" onClick={() => transitionOrder(order.id, "in_repair", "Peça disponível")}>Retomar manutenção</button>}
      {canDelivery && order.status === "ready_for_pickup" && <button className="pt-btn-primary" onClick={() => transitionOrder(order.id, "delivered", "Equipamento entregue ao cliente")}>Confirmar entrega</button>}
