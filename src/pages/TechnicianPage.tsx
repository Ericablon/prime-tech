...
const technicalLabels: Record<TechnicalStatus, string> = {
  not_started: 'Não iniciado', waiting_start: 'Aguardando início', in_progress: 'Em manutenção', paused: 'Pausado',
  waiting_part: 'Aguardando peça', quality_check: 'Testes / qualidade', completed: 'Concluído',
};
const pauseLabels: Record<PauseReason, string> = {
  waiting_part: 'Aguardando peça', waiting_customer: 'Aguardando cliente', waiting_supplier: 'Aguardando fornecedor',
  additional_approval: 'Aguardando aprovação adicional', third_party: 'Serviço de terceiro', observation: 'Observação / testes',
  technical_issue: 'Problema técnico adicional', other: 'Outro motivo',
};
function isToday(value?: string | null) { if (!value) return false; const d = new Date(value); const t = new Date(); return d.getFullYear()===t.getFullYear()&&d.getMonth()===t.getMonth()&&d.getDate()===t.getDate(); }
...
export function TechnicianPage() {
  ...
  async function loadAttachments(orderId: string) {
    if (mode !== 'supabase' || !supabase || !companyId) { setAttachments([]); return; }
    const { data, error: listError } = await supabase.from('service_order_attachments').select('id,file_name,storage_path,note,created_at').eq('company_id',companyId).eq('service_order_id',orderId).order('created_at',{ascending:false});
    if (listError) throw listError;
    const rows = (data ?? []) as Attachment[];
    const withUrls = await Promise.all(rows.map(async (item) => {
      const { data: signed } = await supabase.storage.from('technical-attachments').createSignedUrl(item.storage_path, 3600);
  ...
  async function saveDiagnosis(submit: boolean) {
    ...
    const clean = draft.diagnosis.trim(); const days = Number.parseInt(draft.estimatedDays,10);
  ...
  async function savePause() {
    if (!selectedOrder) return; if (!draft.note.trim()) return setLocalError('Informe o motivo da pausa.');
    setSaving(true); setLocalError('');
  ...
  async function saveUpdate() {
    if (!selectedOrder) return; if (!draft.note.trim()) return setLocalError('Informe a atualização técnica.');
    setSaving(true); setLocalError('');
  ...
  async function uploadPhoto() {
    if (!selectedOrder || !photoFile || !companyId || !user || !supabase || mode !== 'supabase') return;
    ...
    try {
      ...
      const { error: uploadError }=await supabase.storage.from('technical-attachments').upload(path,photoFile,{contentType:photoFile.type,upsert:false});
      if(uploadError) throw uploadError;
      const { error: rowError }=await supabase.from('service_order_attachments').insert({company_id:companyId,service_order_id:selectedOrder.id,uploaded_by:user.id,kind:'photo',file_name:photoFile.name,storage_path:path,mime_type:photoFile.type,file_size:photoFile.size,note:draft.note.trim()||null});
      if(rowError){ await supabase.storage.from('technical-attachments').remove([path]); throw rowError; }
      setPhotoFile(null); setDraft((v)=>({...v,note:''})); await loadAttachments(selectedOrder.id);
    } catch(cause){ const message=cause instanceof Error?cause.message:'Não foi possível salvar a foto.'; setLocalError(message.includes('service_order_attachments')?'Execute a migration 0013 antes de usar fotos técnicas.':message); }
    finally{ setSaving(false); }
  }
  ...
  return <>
    ...
    <div className="tech-summary tech-summary-mobile">
      <article><ClipboardList/><div><strong>{visibleOrders.length}</strong><span>OS disponíveis</span></div></article>
      <article><PlayCircle/><div><strong>{inProgress}</strong><span>Em andamento</span></div></article>
      <article><PackageSearch/><div><strong>{paused}</strong><span>Pausadas</span></div></article>
      <article><CheckCircle2/><div><strong>{completedToday}</strong><span>Concluídas hoje</span></div></article>
    </div>

    {visibleOrders.length===0 ? <section className="panel empty-state tech-empty"><Wrench size={38}/><h3>Nenhuma OS disponível</h3><p>Não existem Ordens de Serviço abertas para este técnico.</p></section> : <div className="mobile-work-list technician-mobile-list">
      {visibleOrders.map((order)=>{
        ...
        return <article className={`work-card technician-work-card ${draft.selectedId===order.id?'active':''}`} key={order.id}>
          <div className="work-card-top"><div><span className="order-number">{orderCode(order.order_number)}</span><h3>{order.client_name??'Cliente não identificado'}</h3><p>{order.equipment??'Equipamento não identificado'}</p></div><StatusBadge status={order.status}/></div>
